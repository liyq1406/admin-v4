            ...
            <div class="nav-aside-content">
              ...
              <div class="nav-aside-group">
                ...
                <template v-for="plugin in plugins" track-by="$index">
                  ...
                  <!-- 非在线维保 -->
                  <div class="nav-aside-item" v-if="plugin.type===10 && plugin.enable===true && plugin.plugin!=='warranty'&&plugin.plugin!=='recipe'"><a v-link="{ path: '/plugins/' + plugin.plugin }"><i class="fa fa-puzzle-piece"></i>{{ plugin.name }}</a></div>
                  <!-- 在线维保 -->
                  <div class="nav-aside-item" v-if="plugin.type===10 && plugin.enable===true && (plugin.plugin==='warranty'||plugin.plugin==='recipe')"><a v-link="{ path: '/plugins/'+ plugin.plugin + '/' +plugin.id }"><i class="fa fa-puzzle-piece"></i>{{ plugin.name }}</a></div>
                </template>
                <div class="nav-aside-actions">
