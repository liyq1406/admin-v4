<template>
  <div class="main">
    <div class="panel mt30">
      <div class="panel-bd ml20 mr20">
        <div class="action-bar">
          <span class="title">{{ $t('account.authorize.authorize_list') }}</span>
          <button class="btn btn-primary" @click.stop.prevent="addKey"><i class="fa fa-plus"></i>{{ $t('account.authorize.add_authorize') }}</button>
        </div>
          ...
          <table class="table table-stripe table-bordered">
            <thead>
              <tr>
                ...
                <th width="18%">{{ $t('account.authorize.create_time') }}</th>
                <th width="14%" class="tac">{{ $t('account.authorize.action') }}</th>
              </tr>
            </thead>
            <tbody>
      ...
      <div slot="body" class="form">
        ...
        <div class="form-actions">
          <label class="del-check">
            <input type="checkbox" name="del" v-model="delChecked"/>{{ $t('account.authorize.del_authorize') }}
          </label>
    ...
    <modal :show.sync="isShowAddModal" @close="onAddCancel">
      <h3 slot="header">{{ $t('account.authorize.add_authorize') }}</h3>
      <div slot="body" class="form">
        <validator name="validation">
          <form autocomplete="off" novalidate @submit.prevent="onAddSubmit">
            <div class="form-row row">
              <label class="form-control col-6">{{ $t('account.authorize.authorize_name') }}:</label>
              <div class="controls col-18">
                <div v-placeholder="$t('account.authorize.placeholders.name')" class="input-text-wrap">
                ...
                <div class="form-tips form-tips-error">
                  <span v-if="$validation.name.touched && $validation.name.required">{{ $t('common.validation.required', {field: $t('account.authorize.fields.name')}) }}</span>
                  <span v-if="$validation.name.touched && $validation.name.modified && $validation.name.minlength">{{ $t('common.validation.minlength', [$t('account.authorize.fields.name'), 2]) }}</span>
                  <span v-if="$validation.name.touched && $validation.name.modified && $validation.name.maxlength">{{ $t('common.validation.maxlength', [$t('account.authorize.fields.name'), 32]) }}</span>
                </div>
              </div>
            </div>
            <div class="form-actions">
