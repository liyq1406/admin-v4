  ...
  <div class="menu-form">
    <div class="form with-loading">
      ...
      <validator name="validation">
        <form novalidate @submit.prevent="onMenuSubmit">
              ...
              <div class="filter-group fr">
                <div class="filter-group-item">
                  <search-box :key.sync="query" :active="searching" :placeholder="$t('ui.ingredient.placeholders.search_condi')" @cancel="getRecipes(true)" @search-activate="toggleSearching" @search-deactivate="toggleSearching" @search="handleSearch" @press-enter="getRecipes(true)">
                    <button slot="search-button" @click="getRecipes(true)" class="btn btn-primary">{{ $t('common.search') }}</button>
                  </search-box>
                </div>
              </div>
              <h3>菜谱列表</h3>
...
export default {
  ...
  ready () {
    if (this.type === 'add') {
      ...
      api.cookbook.getMenus(appId, token, condition).then((res) => {
        if (res.status === 200) {
  ...
  methods: {
    ...
    getRecipes (reset) {
      ...
      api.cookbook.getCookbook(appId, token, this.queryCondition).then((res) => {
        if (res.status === 200) {
          // 虚拟数据开始----------------------------
          // res.data.list = [{
          //   _id: '1231safsdf',
