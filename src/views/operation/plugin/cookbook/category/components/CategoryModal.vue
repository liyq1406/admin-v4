  ...
  methods: {
    ...
    onSubmit () {
      ...
      if (this.type === 'add') {
        ...
        process = api.cookbook.setType(appId, token, params)
      } else {
        if (this.delChecked) {
          // 删除
          process = api.cookbook.delType(appId, this.category._id, token)
        } else {
          //  更新
          process = api.cookbook.editType(appId, this.category._id, token, params)
        }
      }
      process.then((res) => {
        if (res.status === 200) {
