...
export default {
  ...
  data () {
    return {
      breadcrumbNav: [{
        label: '菜谱类别',
        link: `/operation/plugins/cookbook/${this.$route.params.app_id}/categories`
      }, {
        label: '菜谱子类别'
      }],
      columns: [{
  ...
  methods: {
    ...
    getParent () {
      ...
      api.cookbook.getType(appId, token, params).then((res) => {
        if (res.status === 200) {
    ...
    getSubCategories () {
      ...
      api.cookbook.getType(appId, token, this.queryCondition).then((res) => {
        if (res.status === 200) {
