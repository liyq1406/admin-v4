...
export default {
  ...
  methods: {
    ...
    getTags (reset) {
      ...
      api.cookbook.getTags(appId, token, this.queryCondition).then((res) => {
        if (res.status === 200) {
          // 虚拟数据开始----------------------------
          // res.data.list = [{
          //   _id: 'fasfwefe',
    ...
    onSubmit () {
      ...
      if (this.modal.type === 'add') {
        ...
        process = api.cookbook.addTag(appId, token, params)
      } else {
        if (this.delChecked) {
          // 删除
          process = api.cookbook.delTag(appId, this.model._id, token)
        } else {
          //  更新
          process = api.cookbook.editTag(appId, this.model._id, token, params)
        }
      }
      process.then((res) => {
        if (res.status === 200) {
