...
export default {
  ...
  ready () {
    ...
    if (this.type === 'edit') {
      ...
      api.cookbook.getCookbook(appId, token, condition).then((res) => {
        if (res.status === 200) {
  ...
  methods: {
    ...
    getCategories () {
      ...
      api.cookbook.getType(appId, token, condition).then((res) => {
        if (res.status === 200) {
    ...
    getTags () {
      ...
      this.loadingData = true
      api.cookbook.getTags(appId, token, condition).then((res) => {
        if (res.status === 200) {
    ...
    onRecipeSubmit () {
      ...
      if (this.type === 'edit') {
        process = api.cookbook.editCookbook(appId, this.$route.params.id, token, params)
      } else {
        process = api.cookbook.addCookbook(appId, token, params)
      }
