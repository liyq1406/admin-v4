    ...
    methods: {
      ...
      onSetSubmit () {
        if (this.setValidation.$valid && !this.setting) {
          ...
          api.app.empowerWechat(this.currProduct.id, this.$route.params.id, this.setModel).then((res) => {
            if (res.status === 200) {
              this.resetSet()
              this.setting = false
      ...
      showwechat () {
        ...
        api.app.getWechat(this.currProduct.id, this.$route.params.id).then((res) => {
      ...
      createWechat () {
        if (this.addValidation.$valid && !this.adding) {
          api.app.createWechat(this.currProduct.id, this.$route.params.id, this.addModel).then((res) => {
            if (res.status === 200) {
              this.resetAdd()
              this.searchWechatList()
            }
      ...
      empowerStatus () {
        ...
        api.app.productEmpowerStatus(this.currProduct.id, this.$route.params.id).then((res) => {
      ...
      // 产品授权
      productEmpower () {
        api.app.productEmpower(this.currProduct.id, this.$route.params.id).then((res) => {
          if (res.status === 200) {
            this.empowerStatus()
          }
        }).catch((res) => {
