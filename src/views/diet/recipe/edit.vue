                  ...
                  <div v-for="cookingDevice in model.devices" class="select-group">
                    <div class="select inline">
                      <select v-model="cookingDevice.name" @change="setId(cookingDevice)">
                        <option v-for="opt in devices | dropSlected model.devices cookingDevice 'name'" :value="opt.name" :selected="opt.name===cookingDevice.name">{{ opt.name }}</option>
                      </select>
                    </div>
                    <div class="input-text-wrap inline">
...
<script>
  ...
  export default {
    ...
    route: {
      data () {
        api.diet.getRecipe(this.$route.params.id).then((res) => {
          if (res.status === 200) {
            var images = ['', '', '']
    ...
    methods: {
      ...
      deleteRecipe () {
        if (window.confirm('确定要删除该菜谱吗？')) {
          this.editing = true
          api.diet.deleteRecipe(this.$route.params.id).then((res) => {
            if (res.status === 200) {
              this.$route.router.go({path: '/diet/recipe'})
            }
          }).catch((error) => {
