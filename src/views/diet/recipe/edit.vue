...
section.main-wrap.diet
  .main
            ...
            .form-row
              ...
              .controls
                .select-group(v-for="cookingDevice in model.devices")
                  .select.inline
                    select(v-model="cookingDevice.name", @change="setId(cookingDevice)")
                      option(v-for="opt in devices | dropSlected model.devices cookingDevice 'name'", :value="opt.name", :selected="opt.name===cookingDevice.name") {{opt.name}}
                  .input-text-wrap.inline
    ...
    .panel
      .panel-bd
        button.btn.btn-primary.btn-lg.mt10.mb10(@click.prevent="deleteRecipe") {{ $t('recipe.del') }}
</template>
...
export default {
  ...
  methods: {
    ...
    setId (device) {
      device.id = this.devices[_.findIndex(this.devices, (item) => {
