          ...
          <form autocomplete="off" novalidate @submit.prevent="onSubmit">
                ...
                <div class="select">
                  <x-select :label="locales.data.DEVICE_TYPES[editModel.link_type-1]">
                    <select v-model="editModel.link_type" name="link_type">
                      <option v-for="(index, type) in locales.data.DEVICE_TYPES" :value="index+1" :selected="index===0">{{ type }}</option>
                    </select>
                  </x-select>
                </div>
              </div>
            ...
            <div class="form-actions row">
              <div class="col-offset-3">
                <button type="submit" class="btn btn-primary btn-xlg">{{ '提交' }}</button>
                <button type="submit" v-if="!editModel.is_release" class="btn btn-ghost btn-xlg" @click.prevent="deleteProduct">{{ '删除产品' }}</button>
              </div>
            </div>
          </form>
        </validator>
