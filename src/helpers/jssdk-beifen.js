...
(function (win) {
  ...
  function Base64 () {
    ...
    this.encode = function (t) {
      ...
      for (t = o(t); i < t.length;) {
        ...
        c = (15 & e) << 2 | n >> 6
        d = 63 & n
        isNaN(e) ? c = d = 64 : isNaN(n) && (d = 64)
        f = f + r.charAt(h) + r.charAt(C) + r.charAt(c) + r.charAt(d)
      }
      return f
    }
    this.decode = function (o) {
      var a, e, n, h, C, c, d, f
      var i = 0
      for (o = o.replace(/[^A-Za-z0-9\+\/\=]/g, ''); i < o.length;) {
        h = r.indexOf(o.charAt(i++))
        C = r.indexOf(o.charAt(i++))
        c = r.indexOf(o.charAt(i++))
        d = r.indexOf(o.charAt(i++))
        a = h << 2 | C >> 4
        e = (15 & C) << 4 | c >> 2
        n = (3 & c) << 6 | d
        f += String.fromCharCode(a)
        c !== 64 && (f += String.fromCharCode(e))
        d !== 64 && (f += String.fromCharCode(n))
      }
      f = t(f)
      return f
    }
  ...
  var Base = {
    ...
    loadXJSAPILibCallback: this._fn, // 初始化回调
    onXDeviceStateChange: this._fn, // 设备状态发⽣生变化
    onRecvXDeviceData: this._fn, // 接收到设备发送过来的数据
    // 根据deviceid获取设备
  ...
  window['XJSObject'] = (function () {
    var self = __extends(Base, {
      ...
      setSocketList: function (obj) {
        var uri = obj.uri
        var _sock = this.getSocketObj(uri)
        if (_sock) {
          for (var k in obj) {
            if (k !== 'socket') {
              _sock[k] = obj[k]
