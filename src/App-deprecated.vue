          ...
          <div class="nav-aside-wrap">
            <div class="nav-aside-content">
              <div class="nav-aside-item">
                <a v-link="{ path: '/dashboard' }"><i class="fa fa-home"></i>{{ $t("ui.subs.dashboard") }}</a>
              </div>
              <!-- Start: 产品管理 -->
              <div class="nav-aside-group">
                <h3>{{ $t("ui.subs.products") }}</h3>
                <div v-for="product in products" class="nav-aside-item">
                  <a v-link="{ path: '/products/' + product.id" title="{{ product.name }}"><i class="fa fa-link"></i>{{ product.name }}</a>
                </div>
                <div class="nav-aside-actions">
                  <a v-link="{ path: '/product/create' }"><i class="fa fa-plus"></i>{{ $t("ui.subs.add_product") }}</a>
                </div>
              </div>
              <!-- End: 产品管理 -->
              <!-- Start: 插件管理 -->
              <div class="nav-aside-group">
                <h3>{{ $t("ui.subs.plugins") }}</h3>
                <template v-for="plugin in plugins" track-by="$index">
                  ...
                  <!-- 插件 -->
                  <div class="nav-aside-item" v-if="plugin.type===10 && plugin.enable===true"><a v-link="{ path: '/plugins/'+ plugin.plugin + '/' +plugin.id }"><i class="fa fa-puzzle-piece"></i>{{ plugin.name }}</a></div>
                </template>
                <div class="nav-aside-actions">
                  <a v-link="{ path: '/plugins' }"><i class="fa fa-plus"></i>{{ $t("ui.subs.add_plugin") }}</a>
                </div>
              </div>
              <!-- End: 插件管理 -->
