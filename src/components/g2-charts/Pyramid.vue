  ...
  methods: {
    render () {
      ...
      if (this.options.axis === false) {
        ...
        //   chart.axis(key, this.options.axis ? _.merge({}, axisDefaults, this.options.axis[key]) : axisDefaults)
        // }
        chart.axis(_.merge({}, axisDefaults, this.options.axis))
      }
      ...
      var interval = chart.intervalSymmetric().position(this.options.position)
      // var pos = this.options.position

      // if (this.options.stack) {
      //   interval = chart.intervalStack()
      //   pos = G2.Stat.summary.mean(pos)
      // } else {
      ...
      if (this.options.color) {
        interval = interval.color(this.options.color)
      }
      interval.shape('pyramid')
      chart.render()
    }
  }
}
